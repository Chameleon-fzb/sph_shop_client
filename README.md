...
1 点击立即支付,会根据支付信息里面二维码url使用qrcode生成二维码.
2 成功后弹出消息盒子,然后不断轮询,支付的状态
3 当支付成功时,会自动跳转到支付成功页面
4 当支付为成功,那么就停在原地,就算点击我已经成功支付,也不会跳转,但是取消会把轮询停止
5 处理确认和取消的逻辑
  不能使用.then().catch()处理,因为无论有没有支付都会强制的关闭
  使用beforeClose(callback) callback(action,instance,done)
